...
Pertanyaan Refleksi
Apa perbedaan antara @Controller dan @RestController? Dalam kasus apa kamu pakai masing-masing?
...
Perhatikan bahwa template product/list.html dipakai oleh 3 endpoint berbeda (list all, filter by category, search). Apa keuntungannya membuat template yang reusable seperti ini?
...
Kenapa Controller inject ProductService (bukan langsung akses data di ArrayList)? Apa yang terjadi kalau Controller langsung manage data?
...
Apa perbedaan model.addAttribute("products", products) dengan return products langsung seperti di @RestController?
...
Jika kamu buka http://localhost:8080/products/abc (ID bukan angka), apa yang terjadi? Kenapa?
...
Apa keuntungan pakai @RequestMapping("/products") di level class dibanding menulis full path di setiap @GetMapping?
...
Dalam lab ini, kata "Model" muncul dalam beberapa konteks berbeda. Sebutkan minimal 2 arti yang berbeda dan jelaskan perbedaannya.
...
Error resolving fragment: "~{'fragments/layout' :: navbar-yang-salah}": template or fragment could not be resolved (template: "home" - line 13, col 6).
...
CSS masih bekerja? Ya Setelah coba path yang salah:
...
Apa bedanya file di static/ dan templates/? Kenapa CSS ada di static/ bukan templates/?
...
Apa yang dimaksud dengan th:replace dan bagaimana bedanya dengan th:insert?
...
Kenapa kita pakai @{} untuk URL di Thymeleaf, bukan langsung tulis path?
...
Perhatikan bahwa ProductController inject ProductService melalui Constructor Injection (konsep dari Week 3). Apa jadinya kalau Controller tidak pakai DI dan langsung new ProductService() di dalam Controller?
